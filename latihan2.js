...
function rapotAsrama(students) {
    ...
    //Buat jadi berkelompok dulu biar cuma ada 3 grup
    for(i = 0; i < students.length; i++){
        var unique = true
        for(j= 0 ; j < tempClass.length; j++){
            if(students[i]['asrama'] === tempClass[j]['asrama']){
                unique = false
            }
        }
        if(unique){
            tempClass.push({asrama : students[i]['asrama'], gagal : [], lulus: []})
        }
    }


    //disini baru masukin nilainya
    for(i = 0 ; i < tempClass.length; i++){
        for(j = 0; j < students.length; j++){
            if(tempClass[i]['asrama'] === students[j]['asrama']){
                if(students[j]['nilai'] > 70){
                    tempClass[i]['lulus'].push(students[j]['nama'])
                }else{
                    tempClass[i]['gagal'].push(students[j]['nama'])
                }
            }
        }
    }
